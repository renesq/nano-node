...
std::error_code nano::handle_node_options (boost::program_options::variables_map const & vm)
{
	...
	else if (vm.count ("timestamps_update_frontiers") == 1)
	{
		...
		{
			auto block (node.node->store.block_get (transaction, i->second.head));
